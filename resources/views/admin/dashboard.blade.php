<x-sidebar-layout>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        ...
        <div class="glass-panel dark:bg-slate-800/80 p-6 rounded-2xl relative overflow-hidden group hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1">
            <div class="absolute -right-6 -top-6 w-24 h-24 bg-amber-500/10 rounded-full blur-xl group-hover:bg-amber-500/20 transition-all"></div>
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-slate-500 dark:text-slate-400 font-medium text-sm">Unassigned</h3>
                <span class="w-8 h-8 rounded-lg bg-amber-50 text-amber-500 flex items-center justify-center">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                </span>
            </div>
            <p class="text-3xl font-bold text-slate-800 dark:text-white">{{ $proposals->whereNull('reviewer_id')->count() }}</p>
        </div>
    ...
    <div class="glass-panel rounded-3xl overflow-hidden shadow-xl shadow-slate-200/50 dark:shadow-none border border-white/60 dark:border-white/10 dark:bg-slate-800/80" x-data="{ search: '', filter: 'all', filterOpen: false }">
        <div class="p-8 border-b border-slate-100/50 dark:border-slate-700 bg-white/40 dark:bg-slate-800/40 flex justify-between items-center flex-wrap gap-4">
            <div>
                <h2 class="text-xl font-bold text-slate-800 dark:text-white">All Proposals Overview</h2>
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Manage proposals and assign reviewers system-wide.</p>
            </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50/50 text-slate-500 text-xs uppercase tracking-wider">
                        <th class="px-8 py-5 font-semibold">Title / Proposer</th>
                        <th class="px-8 py-5 font-semibold">Reviewer</th>
                ...
                <tbody class="divide-y divide-slate-100/50 dark:divide-slate-700/50 bg-white/60 dark:bg-slate-900/50">
                    ...
                    <tr x-show="(filter === 'all' || '{{ $proposal->status }}' === filter) && (search === '' || $el.textContent.toLowerCase().includes(search.toLowerCase()))" class="hover:bg-amber-50/30 dark:hover:bg-slate-800 transition-colors duration-200 group">
                        <td class="px-8 py-5">
                            <div class="text-sm font-semibold text-slate-800 dark:text-slate-200 group-hover:text-amber-600 dark:group-hover:text-amber-500 transition-colors truncate max-w-xs">
                                {{ $proposal->title }}
                            </div>
                            <div class="text-xs text-slate-500 dark:text-slate-400 mt-1">
                                {{ $proposal->proposer->name }} &bull; {{ $proposal->category }}
                            </div>
                        </td>
                        <td class="px-8 py-5 text-sm">
                            @if($proposal->reviewer)
                                <span class="inline-flex items-center text-emerald-700 bg-emerald-50 dark:bg-emerald-500/20 dark:text-emerald-400 px-2.5 py-1 rounded-lg ring-1 ring-inset ring-emerald-600/20 dark:ring-emerald-500/30 font-medium text-xs">
                            ...
                            @else
                                <span class="inline-flex items-center text-rose-700 bg-rose-50 dark:bg-rose-500/20 dark:text-rose-400 px-2.5 py-1 rounded-lg ring-1 ring-inset ring-rose-600/20 dark:ring-rose-500/30 font-medium text-xs">
                                    <svg class="w-3.5 h-3.5 mr-1.5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd"></path></svg>
                                    Unassigned
                                </span>
                            @endif
                        </td>
                        ...
                        <td class="px-8 py-5 text-right">
                            <a href="{{ route('admin.proposal.show', $proposal) }}" class="inline-flex items-center justify-center p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 dark:hover:bg-slate-700 rounded-xl transition-all">
                    ...
                    <tr>
                        <td colspan="5" class="px-8 py-16 text-center">
                            ...
                            <h3 class="text-sm font-medium text-slate-900">No proposals found</h3>
                            <p class="mt-1 text-sm text-slate-500 mb-6">There are no proposals in the system yet.</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
