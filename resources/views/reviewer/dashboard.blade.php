<x-sidebar-layout>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div class="glass-panel dark:bg-slate-800/80 p-6 rounded-2xl relative overflow-hidden group hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1">
            ...
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-slate-500 dark:text-slate-400 font-medium text-sm">Total Assigned</h3>
        ...
        <div class="glass-panel dark:bg-slate-800/80 p-6 rounded-2xl relative overflow-hidden group hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1">
            <div class="absolute -right-6 -top-6 w-24 h-24 bg-amber-500/10 rounded-full blur-xl group-hover:bg-amber-500/20 transition-all"></div>
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-slate-500 dark:text-slate-400 font-medium text-sm">Pending Review</h3>
                <span class="w-8 h-8 rounded-lg bg-amber-50 text-amber-500 flex items-center justify-center">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                </span>
            </div>
            <p class="text-3xl font-bold text-slate-800 dark:text-white">{{ $proposals->where('status', 'pending_review')->count() }}</p>
        </div>
        ...
        <div class="glass-panel dark:bg-slate-800/80 p-6 rounded-2xl relative overflow-hidden group hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1">
            ...
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-slate-500 dark:text-slate-400 font-medium text-sm">Reviewed</h3>
            ...
            <p class="text-3xl font-bold text-slate-800 dark:text-white">{{ $proposals->whereIn('status', ['approved', 'rejected', 'revision_required'])->count() }}</p>
        </div>
    </div>
    ...
    <div class="glass-panel rounded-3xl overflow-hidden shadow-xl shadow-slate-200/50 dark:shadow-none border border-white/60 dark:border-white/10 dark:bg-slate-800/80" x-data="{ search: '', filter: 'all', filterOpen: false }">
        <div class="p-8 border-b border-slate-100/50 dark:border-slate-700 bg-white/40 dark:bg-slate-800/40 flex justify-between items-center flex-wrap gap-4">
            <div>
                <h2 class="text-xl font-bold text-slate-800 dark:text-white">Assigned Proposals</h2>
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Review the proposals assigned to you.</p>
            </div>
            ...
            <div class="flex items-center space-x-3 w-full md:w-auto">
                <div class="relative flex-1 md:w-64">
                    ...
                    <input x-model="search" type="text" class="block w-full pl-10 pr-3 py-2 text-sm border-0 ring-1 ring-slate-200 dark:ring-slate-700 focus:ring-2 focus:ring-amber-500 rounded-xl bg-white/80 dark:bg-slate-900/50 dark:text-slate-200 placeholder-slate-400 transition-shadow" placeholder="Search assigned...">
                </div>
                <div class="relative">
                ...
                <tbody class="divide-y divide-slate-100/50 dark:divide-slate-700/50 bg-white/60 dark:bg-slate-900/50">
                    ...
                    <tr x-show="(filter === 'all' || '{{ $proposal->status }}' === filter) && (search === '' || $el.textContent.toLowerCase().includes(search.toLowerCase()))" class="hover:bg-amber-50/30 dark:hover:bg-slate-800 transition-colors duration-200 group">
                        ...
                        <td class="px-8 py-5 text-right">
                            <a href="{{ route('reviewer.proposal.show', $proposal) }}" class="inline-flex items-center justify-center p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 dark:hover:bg-slate-700 rounded-xl transition-all">
                    ...
                    <tr>
                        <td colspan="5" class="px-8 py-16 text-center">
                            ...
                            <h3 class="text-sm font-medium text-slate-900">No assigned proposals</h3>
                            <p class="mt-1 text-sm text-slate-500 mb-6">You currently have no proposals assigned for review.</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
