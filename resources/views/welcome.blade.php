...
<body class="antialiased bg-gray-50 text-gray-900 selection:bg-amber-500 selection:text-white">
    <div class="relative min-h-screen flex flex-col items-center justify-center overflow-hidden">
        ...
        <div class="relative z-10 w-full max-w-5xl px-6 py-12 mx-auto text-center">
            ...
            <!-- Navbar / Login Links -->
            @if (Route::has('login'))
                <div class="absolute top-6 right-6 sm:top-8 sm:right-8 flex space-x-4">
                    @auth
                        <a href="{{ route('dashboard') }}"
                            class="font-semibold text-gray-600 hover:text-amber-600 focus:outline focus:outline-2 focus:rounded-sm focus:outline-amber-500 transition-colors">Dashboard</a>
                    @else
                        <a href="{{ route('login') }}"
                            class="font-semibold text-gray-600 hover:text-amber-600 focus:outline focus:outline-2 focus:rounded-sm focus:outline-amber-500 transition-colors">Log
                            in</a>

                        @if (Route::has('register'))
                            <a href="{{ route('register') }}"
                                class="font-semibold text-gray-600 hover:text-amber-600 focus:outline focus:outline-2 focus:rounded-sm focus:outline-amber-500 transition-colors">Register</a>
                        @endif
                    @endauth
                </div>
            @endif
            ...
            <div class="mb-12">
                ...
                <h1
                    class="text-5xl md:text-6xl font-extrabold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-amber-700 to-zinc-700 mb-4">
                    JDH Edu Tracker
                </h1>
            ...
            <div class="flex flex-col sm:flex-row items-center justify-center gap-4">
                @auth
                    <a href="{{ route('dashboard') }}"
                        class="w-full sm:w-auto inline-flex justify-center items-center px-8 py-4 text-base font-bold text-white bg-amber-600 border border-transparent rounded-full shadow-lg hover:bg-amber-700 hover:shadow-amber-500/30 transition-all duration-300 hover:-translate-y-1">
                        Go to Dashboard
                    </a>
                @else
                    <a href="{{ route('login') }}"
                    ...
                    <a href="{{ route('register') }}"
                        class="w-full sm:w-auto inline-flex justify-center items-center px-8 py-4 text-base font-bold text-gray-700 bg-white border border-gray-200 rounded-full shadow hover:bg-gray-50 transition-all duration-300 hover:-translate-y-1">
                        Create Account
                    </a>
                @endauth
            </div>
            ...
            <div class="mt-24 grid grid-cols-1 md:grid-cols-3 gap-8">
                <!-- Feature 1 -->
                <div
                    class="bg-white/60 backdrop-blur-lg rounded-3xl p-8 border border-white shadow-xl text-left hover:-translate-y-2 transition-transform duration-300">
